...
def googleTop(quote, metadata, u):
    ...
    metadata_query = urllib.urlencode(' '.join(metadata.keywords[:NUM_KEYWORDS_TO_USE]) + ' ' + ' '.join(metadata.entities[:NUM_ENTITIES_TO_USE]))
    ...
    try:
        ...
        # pprint.pprint(req)
        # res = req.execute()
        url = urllib.urlencode("https://www.googleapis.com/customsearch/v1?q=" + metadata_query + "&cx=006173695502366383915%3Acqpxathvhhm&exactTerms=" + quote + "&key=AIzaSyABOiui8c_-sFGJSSXCk6tbBThZT2NI4Pc")
        res = json.loads((urllib.urlopen(url)).read())
        print "URL"
        print url
        print "RES"
